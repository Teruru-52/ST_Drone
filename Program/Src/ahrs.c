...
//相補フィルターで補正した角速度を用いてクォータニオンを求める
void ahrs_fusion_ag(AxesRaw_TypeDef_Float *acc, AxesRaw_TypeDef_Float *gyro, AHRS_State_TypeDef *ahrs)
{
  ...
  if(gTHR<MIN_THR) // flight.control.h  MIN_THR:DC motor configuration
  {
    ahrs_kp = AHRS_KP_BIG;
  }
  else
  ...
  // 姿勢クォータニオンの前回値から機体座標系での重力加速度ベクトルを推定する
  vx = 2*(q1q3 - q0q2);
  vy = 2*(q0q1 + q2q3);
  vz = q0q0 - q1q1 - q2q2 + q3q3;

  // 姿勢の推定誤差ベクトル
  // [vx, vy, vz]を[axf, ayf, azf]に近づけるための回転軸ベクトル（外積をとっている）
  ex = (ayf*vz - azf*vy);
  ey = (azf*vx - axf*vz);
  ez = (axf*vy - ayf*vx);
